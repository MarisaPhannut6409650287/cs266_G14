...
describe("Review Restaurant Feature", function () {
  ...
  it("TC01: If user login and submits a review, displayed review must show the username used for login.", async function () {
    ...
    try {
      ...
      const dropdown2 = await driver.wait(until.elementLocated(By.id('inputGroupSelect02')), 5000);
      const dropdownOption2 = await driver.wait(until.elementLocated(By.css('option[value="3"]')), 5000);
  ...
  it("TC02: If user not login and submits a review, displayed review must show the username is 'Anonymous'.", async function () {
    ...
    try {
      ...
      const dropdown2 = await driver.wait(until.elementLocated(By.id('inputGroupSelect02')), 5000);
      const dropdownOption2 = await driver.wait(until.elementLocated(By.css('option[value="5"]')), 5000);
  ...
  it("TC04: If user enters review detail, system should will show detail as the user entered it and date the user send.", async function () {
    this.timeout(10000000);
    ...
    try {
      ...
      const displayPoint = await driver.findElement(By.id('pointUser'));
      const pointText = await displayPoint.getText();
      const pointValue = parseInt(pointText);
      assert.equal(pointValue, 5);

      const displayType = await driver.findElement(By.id('commentType'));
      const typeText = await displayType.getText();
      assert.equal(typeText, 'Food');

      const displayDetail = await driver.findElement(By.id('word'));
      const detailText = await displayDetail.getText();
      assert.equal(detailText, 'delicious!!!');

      const displayTime = await driver.findElement(By.id('time'));
      const timeText = await displayTime.getText();
      assert.equal(timeText, '2023-12-05');
  ...
  it("TC05: If have multi review, system should calculate average from point of user and count of review correct.", async function () {
    this.timeout(10000000);
